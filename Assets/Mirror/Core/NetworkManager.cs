    ...
    public class NetworkManager : MonoBehaviour
    {
        ...
        public virtual void Start()
        {
            ...
            if (Utils.IsHeadless())
            {
                ...
                {
#if !UNITY_WEBGL
                    // WebGL builds can't be servers, so don't try to auto-start server
                    if (headlessStartMode == HeadlessStartOptions.AutoStartServer)
                    {
                        StartServer();
                    }
                    else if (headlessStartMode == HeadlessStartOptions.AutoStartClient)
                    {
                        StartClient();
                    }
#else
                    // For WebGL, only allow AutoStartClient
                    if (headlessStartMode == HeadlessStartOptions.AutoStartClient)
                    {
                        StartClient();
                    }
#endif
